/**
 * UbaVoy - Service Worker de la app del CLIENTE
 * ===========================================================================
...
self.addEventListener('install', (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(RECURSOS.map(async (url) => {
      ...
      catch (e) { console.warn('[SW client] no se pudo precargar', url); }
    }));
  ...
  if (req.mode === 'navigate' || (req.headers.get('accept') || '').includes('text/html')) {
    event.respondWith((async () => {
      try {
        ...
        const cache = await caches.open(CACHE_NAME);
        cache.put('/apps/client/', res.clone());
        return res;
      } catch (e) {
        return (await caches.match('/apps/client/')) ||
